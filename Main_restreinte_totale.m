function R=main_restreinte_totale(s, t0, E28, nE, Retrait_endogene28, ...
    nretrait_endogene, ft28, nft, rho)
    ...
    % Laitier activé
    % >> s = 0.374  ;
    % >> t0 =  0.114*24*3600.; % Parametres communs [] et temps prise [s]
    % >> E28 = 22.8E9 ; 
    % >> nE = 0.546 ; % [Pa]
    % >> Retrait_endogene28 = -204.E-6 ; 
    % >> nretrait_endogene = 4.44;
    Retrait_dessiccation28 = 0. * (-457.E-6) ; 
    kt = 0.0343 ; ks = 1.25 ;
    % >> ft28 = 3.01E6 ; 
    % >> nft = 0.808;
    ...
    t28 = 28.*24 * 3600 ;
    E_kv28 = 1.E20* 21.1141*1.E9 ; 
    n_E_kv = 0.3 ;
    Eta_kv28 = 1.E20*6.82*24*3600 * E_kv28 ; 
    n_Eta_kv = 0.3 ;
    ...
    parameters_material = zeros(6,1); 
    affinity = zeros(3,1) ; 
    conditions = zeros(4,1) ;
    parameters_material(1) = rho; %2188 ;                % Masse volumique kg/m3
    ...
    npastemps = 200; 
    dt = (tfin - t0) / npastemps;
    
    % Initialisation des variables
    % FLUAGE
    sigma_fluage = zeros(npastemps + 1,1) ; 
    sigma_elas = zeros(npastemps + 1,1) ; 
    sigma_fluage_amor = zeros(npastemps + 1,1) ;
    ...
    ft = zeros(npastemps + 1,1) ; 
    E = zeros(npastemps + 1,1) ;
    % Indice de Sensibilité à la Fissuration (Cracking Index)
    ISF_fluage = zeros(npastemps + 1,1) ; 
    ISF_fluage_amor = zeros(npastemps + 1,1) ;
    ISF_elas = zeros(npastemps + 1,1) ;
    % Autre vecteurs
    temps_eq = zeros(npastemps + 1,1) ; 
    temps_eq(1) = 0; % Temps équivalent
    Tmoy = zeros(npastemps + 1,1) ; % Température moyenne
    Real_Time = zeros(npastemps + 1,1) ; 
    Real_Time(1) = 0; % Temps réel
